 ...
 */
    function valid() {
    	$this->log(env('User-Agent'));
        if ($this->__active === true) {
            if ($this->read('Config')) {
                if (($this->_userAgent == $this->read("Config.userAgent") OR $this->__isFlashAgent()) &&
 ...
 */
    ...
    function __isFlashAgent() {
    	$flashAgent = array('Shockwave Flash','Adobe Flash Player 9','Adobe Flash Player 8');
    	foreach ($flashAgent as $agent) {
    		if (md5($agent. CAKE_SESSION_STRING) == $this->_userAgent)
    		return true;
    	}
    }
