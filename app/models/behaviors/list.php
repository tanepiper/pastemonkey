...
class ListBehavior extends ModelBehavior
{
  ...
  */
	...
	/**
	 * set_insert
	 * A setter function to allow new items to be inserted at top or
	 * bottom of the list at runtime.
	 * 
	 * @param object $model    Model whos settings are about to be changed.
	 * @param String $position	New position value
	 * 
	 *  @access public
	 */

	function set_insert_possition(&$model, $position = null)
	{
		if($position != null){
			if($position == 'top') {
				$this->settings[$model->name]['position'] = 'top';
			} else {
				$this->settings[$model->name]['position'] = 'bottom';
			}		
		} else {
			return false;
		}
	...
	function beforeSave(&$model)
	{
		...
		if(empty($model->{$model->primaryKey}))
		{
			...
			else
			{
				...
				$model->data[$model->name][$field] = $item[$model->name][$field] + 1;
			}
		}
	}
	...
	/**
     * Run before a model is deleted, removes the model from the list
     * updating the rest of the orders
     *
	...
	function beforeDelete(&$model)
	{
		...
		$item = $model->read(null,$model->id);
		
		$conditions = $this->_get_conditions($model);
		$conditions["$field"] = "> {$item[$model->name][$field]}";
     ...
     */	
	function moveTo(&$model, $id, $target_position)
	{
		extract($this->settings[$model->name]);
	 ...
	 */
	function _get_conditions(&$model, $id = null)
	{
		$conditions = array();
		if($id != null){
