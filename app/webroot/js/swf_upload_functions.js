...
function uploadComplete(fileObj, server_data) {
	try {	
		...
		if(server_data.length > 500){
			progress.SetError();
			progress.SetStatus("Upload Failed - Session expired, you are no loger logged in.");
			this.debugMessage("Error Code: Upload Failed, File name: " + file.name + ", File size: " + file.size + ", Message: User no loger logged in.");
		}else if(server_data.toString() == 'true'){
			progress.SetComplete();
			progress.SetStatus("Complete.");
			progress.ToggleCancel(false);
		}else{
			progress.SetError();
			progress.SetStatus("Upload Failed - "+server_data);
			this.debugMessage("Error Code: Upload Failed, File name: " + file.name + ", File size: " + file.size + ", Message: ". server_data);
		}
...
function uploadQueueComplete(fileObj) {
	try {
		...
		var model = this.settings.post_params.model;
		...
		if($pastemonkey('#admin_attachments_'+model+'_'+model_id+'_'+group).is('ul')){
			...
			if(confirm('Upload is complete, re-load the page and see the result (accept) or continue to edit (reject)')){
