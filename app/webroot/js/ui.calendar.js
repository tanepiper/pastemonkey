...
function PopUpCal() {
	...
	$pastemonkey.extend(this._defaults, this.regional['']);
	this._calendarDiv = $pastemonkey('<div id="calendar_div"></div>');
	$pastemonkey(document.body).append(this._calendarDiv);
	$pastemonkey(document.body).mousedown(this._checkExternalClick);
}

$pastemonkey.extend(PopUpCal.prototype, {
	...
	_connectCalendar: function(target, inst) {
		...
		if (autoPopUp == 'button' || autoPopUp == 'both') { // pop-up calendar when button clicked
			...
			var trigger = $pastemonkey(buttonImageOnly ? '<img class="calendar_trigger" src="' +
	...
	dialogCalendar: function(dateText, onSelect, settings, pos) {
		...
		if (!inst) {
			...
			this._dialogInput = $pastemonkey('<input type="text" size="1" style="position: absolute; top: -100px;"/>');
			this._dialogInput.keydown(this._doKeyDown);
			$pastemonkey('body').append(this._dialogInput);
		...
		if ($pastemonkey.blockUI) {
			$pastemonkey.blockUI(this._calendarDiv);
		}
	},
	...
	enableFor: function(inputs) {
		inputs = (inputs.jquery ? inputs : $pastemonkey(inputs));
		inputs.each(function() {
			this.disabled = false;
			$pastemonkey('../button.calendar_trigger', this).each(function() { this.disabled = false; });
			$pastemonkey('../img.calendar_trigger', this).css({opacity:'1.0',cursor:''});
			var $pastemonkeythis = this;
			popUpCal._disabledInputs = $pastemonkey.map(popUpCal._disabledInputs,
				function(value) { return (value == $pastemonkeythis ? null : value); }); // delete entry
		});
	},
	...
	disableFor: function(inputs) {
		...
		inputs.each(function() {
			this.disabled = true;
			$pastemonkey('../button.calendar_trigger', this).each(function() { this.disabled = true; });
			$pastemonkey('../img.calendar_trigger', this).css({opacity:'0.5',cursor:'default'});
			var $pastemonkeythis = this;
			popUpCal._disabledInputs = $pastemonkey.map(popUpCal._disabledInputs,
				function(value) { return (value == $pastemonkeythis ? null : value); }); // delete entry
	...
	reconfigureFor: function(control, settings) {
		control = (typeof control == 'string' ? $pastemonkey(control)[0] : control);
	...
	showFor: function(target) {
		...
		if (input.nodeName.toLowerCase() != 'input') { // find from button/image trigger
			input = $pastemonkey('input', input.parentNode)[0];
		}
		...
		inst._calendarDiv.css('position', (popUpCal._inDialog && $pastemonkey.blockUI ? 'static' : 'absolute')).
	...
	_afterShow: function(inst) {
		if ($pastemonkey.browser.msie) { // fix IE < 7 select problems
			$pastemonkey('#calendar_cover').css({width: inst._calendarDiv[0].offsetWidth + 4,
		...
		if (popUpCal._popUpShowing) {
			...
			if (popUpCal._inDialog) {
				...
				if ($pastemonkey.blockUI) {
					...
					$pastemonkey('body').append(this._calendarDiv);
				}
			}
	...
	_tidyDialog: function(inst) {
		...
		$pastemonkey('.calendar_prompt', inst._calendarDiv).remove();
	},
	...
	_clickMonthYear: function(id) {
		var inst = this._getInst(id);
		if (inst._input && inst._selectingMonthYear && !$pastemonkey.browser.msie) {
			inst._input[0].focus();
		}
	...
	_selectDay: function(id, td) {
		var inst = this._getInst(id);
		inst._selectedDay = $pastemonkey("a", td).html();
		this._selectDate(id);
	},
...
function PopUpCalInstance(settings, inline) {
	...
	this._calendarDiv = (!inline ? popUpCal._calendarDiv :
		$pastemonkey('<div id="calendar_div_' + this._id + '" class="calendar_inline"></div>'));
...
$pastemonkey.extend(PopUpCalInstance.prototype, {
	...
	_generateCalendar: function() {
		...
		for (var row = 0; row < numRows; row++) { // create calendar rows
			...
			for (var dow = 0; dow < 7; dow++) { // create calendar days
				...
				html += '<td class="calendar_daysCell' +
					...
					(unselectable ? '' : ' onmouseover="$pastemonkey(this).addClass(\'calendar_daysCellOver\');"' +
					' onmouseout="$pastemonkey(this).removeClass(\'calendar_daysCellOver\');"' +
		...
		html += '</tbody></table>' + (!closeAtTop && !this._inline ? controls : '') +
			'<div style="clear: both;"></div>' + (!$pastemonkey.browser.msie ? '' :
...
function extendRemove(target, props) {
	$pastemonkey.extend(target, props);
...
$pastemonkey.fn.calendar = function(settings) {
	return this.each(function() {
		...
		if (nodeName == 'input') {
			var instSettings = (inlineSettings ? $pastemonkey.extend($pastemonkey.extend({}, settings || {}),
			...
			var instSettings = $pastemonkey.extend($pastemonkey.extend({}, settings || {}),
